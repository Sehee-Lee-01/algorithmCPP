...
// 최적경로 찾을 때 이용
const int direc_y[] = {-1, 0, 0, 1};
const int direc_x[] = {0, -1, 1, 0};
...
public:
    ...
    void moveRoad(const vector<vector<int>> &map, const int n)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            int road_y = curr_place.first + direc_y[j];
            int road_x = curr_place.second + direc_x[j];
            ...
            else if (!((past_place.first == road_y) && (past_place.second == road_x))) // 이전에 갔던 곳이 아닌지
            {
                if ((0 <= road_y) && (road_y < n) && (0 <= road_x) && (road_x < n) && (map[road_y][road_x] != -1))
                {
                    int dis = abs(road_y - sung_place.first) + abs(road_x - sung_place.second);
                    if (min_dis > dis) // 최소 거리인지
                    {
                        move = make_pair(road_y, road_x);
                        min_dis = dis;
                    }
                }
            }
